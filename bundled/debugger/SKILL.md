...
`server.dispatch_prompt('debugger_quick.md')` 로 단일 sub-agent dispatch:

```python
prompt = server.dispatch_and_record(
    run_id,
    prompt_file="debugger_quick.md",
    step="debugger.quick",
    description="debugger quick mode — single-dispatch fallback",
)
